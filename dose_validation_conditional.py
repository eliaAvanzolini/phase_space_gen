"""
dose_validation_conditional.py
==============================
VERSIONE SCIENTIFICA DEFINITIVA - SUPPORTO INTEGRATORE ADATTIVO DOPRI5
"""
...
def _load_cfm(checkpoint_path: str, dim: int, device: str):
    ...
    hidden_dim = 256
    if "velocity_net.input_proj.weight" in sd: hidden_dim = sd["velocity_net.input_proj.weight"].shape[0]
    n_layers = 4
    max_idx  = -1
    for k in sd:
    ...
    if max_idx >= 0: n_layers = max_idx + 1
...
def _load_nsf(checkpoint_path: str, dim: int, device: str):
    ...
    hidden_dim = 256
    for k, v in sd.items():
        if "transform_net.initial_layer.weight" in k: hidden_dim = v.shape[0]; break
    n_transforms = 6
    max_idx = -1
    for k in sd:
        if "_transforms." in k:
            s = k.split("_transforms.")[1].split(".")[0]
            if s.isdigit(): max_idx = max(max_idx, int(s))
    if max_idx >= 0: n_transforms = (max_idx + 1) // 2
...
def _load_gan_auto(checkpoint_path: str, cond_dim: int, out_dim: int, device: str):
...
def generate_cfm_nsf(model_cfg: dict, n_samples: int, cond_vector: list, device: str, cfm_steps: int, solver: str) -> np.ndarray:
    ...
    with open(cond_stats_path) as f: cond_stats = json.load(f)
        ...
        while n_done < n_samples:
            ...
            if is_cfm:
                ...
                if solver == "euler":
                    ...
                    # Chiamata al metodo .sample() nativo che usa dopri5
...
def generate_gan_sarrut(model_cfg: dict, n_samples: int, cond_vector: list, device: str) -> np.ndarray:
    from data.synthetic_linac import denormalize_phase_space
    stats_path = model_cfg["stats_json"]
    cond_stats_path = Path(model_cfg["stats_json"]).parent / "condition_stats.json"
    with open(stats_path) as f: stats = json.load(f)
    ...
    ckpt = torch.load(model_cfg["checkpoint"], map_location=device, weights_only=False)
    state_dict = ckpt.get("generator") or ckpt.get("G") or ckpt
    clean_state = {k.replace("model.", ""): v for k, v in state_dict.items()}
    out_dim = clean_state["head.weight"].shape[0] if "head.weight" in clean_state else 6
    G = _load_gan_auto(model_cfg["checkpoint"], cond_dim=3, out_dim=out_dim, device=device)
    ...
    with torch.no_grad():
        while n_done < n_samples:
            ...
            cond_tensor = torch.tensor(cond_norm, dtype=torch.float32, device=device).unsqueeze(0).repeat(n_chunk, 1)
            g_norm = G(z, cond_tensor).cpu().numpy()
            chunks_6d.append(g_norm)
            n_done += n_chunk
            del z, cond_tensor, g_norm
            if device == "cuda": torch.cuda.empty_cache()
            
    gen_6d = np.concatenate(chunks_6d, axis=0)
    ps_out = denormalize_phase_space(gen_6d, stats).astype(np.float32)
    
    if ps_out.shape[1] == 6:
        gen_7d = np.zeros((len(ps_out), 7), dtype=np.float32)
        gen_7d[:, [0, 1, 3, 4, 5, 6]] = ps_out
        gen_7d[:, 2] = stats.get("z_const", 0.0)
        return gen_7d
    return ps_out
...
def _apply_physical_filters(ps7: np.ndarray, model_name: str) -> np.ndarray:
    ...
    if mask.sum() == 0: return ps7[:10].copy()
    ps7_clean = ps7[mask].copy()
    norms = np.sqrt(ps7_clean[:, 3]**2 + ps7_clean[:, 4]**2 + ps7_clean[:, 5]**2)
    ps7_clean[:, 3] /= norms; ps7_clean[:, 4] /= norms; ps7_clean[:, 5] /= norms
    return ps7_clean
...
def run_gate_dose(source_path: Path, output_dir: Path, run_name: str, n_threads: int, voxel_mm: float):
    ...
    dose.attached_to, dose.size, dose.spacing, dose.output_filename = water.name, [n_vox, n_vox, n_vox], [voxel_mm * mm, voxel_mm * mm, voxel_mm * mm], str(output_dir / f"dose_{run_name}.mhd")
    dose.hit_type, dose.dose.active, dose.dose_uncertainty.active = "random", True, True
    src = sim.add_source("PhaseSpaceSource", "phsp_src")
    src.phsp_file, src.particle, src.n = str(source_path), "gamma", actual_n
...
def main():
    ...
    p.add_argument("--field", choices=["6mv_5x5", "10mv_20x20"], required=True)
    p.add_argument("--n_particles", type=int, default=10000000)
    ...
    p.add_argument("--solver", choices=["euler", "dopri5"], default="euler", help="Tipo di solutore ODE")
    args = p.parse_args()

    fields_map = {"6mv_5x5": [6.0, 2.5, 2.5], "10mv_20x20": [10.0, 10.0, 10.0]}
    ...
    if args.subtask == "reference":
        ...
        with h5py.File(PHSP2_PATH, "r") as f:
            ps_all = f["phase_space"][:]; cond_all = f["conditions"][:]
        mask = (np.abs(cond_all[:, 0] - cond_vector[0]) < 0.1) & (np.abs(cond_all[:, 1] - cond_vector[1]) < 0.1) & (np.abs(cond_all[:, 2] - cond_vector[2]) < 0.1)
        available_indices = np.where(mask)[0]
        pool_size = len(available_indices)
        
        if args.n_particles > pool_size: n_to_sample = pool_size
        else: n_to_sample = args.n_particles

        chosen_indices = np.random.choice(available_indices, size=n_to_sample, replace=False)
        ps_ref = ps_all[chosen_indices].astype(np.float32)
        
        with uproot.recreate(ref_root) as f:
            f["PhaseSpace"] = {"X": ps_ref[:, 0]*10.0, "Y": ps_ref[:, 1]*10.0, "Z": np.zeros(len(ps_ref), dtype=np.float32), "dX": ps_ref[:, 3], "dY": ps_ref[:, 4], "dZ": np.abs(ps_ref[:, 5]).astype(np.float32), "E": ps_ref[:, 6]}
        run_gate_dose(ref_root, out, run_name, args.n_threads, args.voxel_mm)
    else:
        name = args.subtask
        rpath = out / f"gen_{run_name}.root"
        ps7 = generate_gan_sarrut(MODELS[name], args.n_particles, cond_vector, dev) if name == "gan" else generate_cfm_nsf(MODELS[name], args.n_particles, cond_vector, dev, args.cfm_steps, args.solver)
        ps7_clean = _apply_physical_filters(ps7, name)
        with uproot.recreate(rpath) as f:
            f["PhaseSpace"] = {"X": ps7_clean[:, 0]*10.0, "Y": ps7_clean[:, 1]*10.0, "Z": np.zeros(len(ps7_clean), dtype=np.float32), "dX": ps7_clean[:, 3], "dY": ps7_clean[:, 4], "dZ": np.abs(ps7_clean[:, 5]).astype(np.float32), "E": ps7_clean[:, 6]}
